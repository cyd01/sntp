...
func init() {
	...
	speed_time = 0
	fmt.Println("Init time is ", init_time)
}

func SetSpeedTime(s int64) {
	speed_time = s
	fmt.Println("Set speed time to ", s+1)
}
...
func generate(req []byte) []byte {
	real_time := time.Now()
	var second = unix2ntp(real_time.Unix())

	if speed_time > 0 {
		offset := speed_time * (real_time.Unix() - init_time)
		second = second + offset
		fmt.Println("	offset=", offset)
	}

	var fraction = unix2ntp(int64(real_time.Nanosecond()))
